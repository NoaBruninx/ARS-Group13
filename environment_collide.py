...
def main():
    ...
    while running:
        dt_actual = clock.tick(60) / 1000.0
        dt_actual = min(dt_actual, 0.05)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # Increase noise
                    ...
                    NOISE_LEVEL = min(NOISE_LEVEL + 0.5, 20)

                # Decrease noise
                elif event.key == pygame.K_MINUS:
                    NOISE_LEVEL = max(NOISE_LEVEL - 0.5, 0)
        ...
        # -----------------------------
        # APPLY NOISE ONLY WHEN MOVING
        # -----------------------------
        is_moving = abs(v_left) > 0.01 or abs(v_right) > 0.01

        if is_moving and NOISE_LEVEL > 0:
            v += random.gauss(0, NOISE_LEVEL)
            omega += random.gauss(0, NOISE_LEVEL * 0.01)
        ...
        # HUD (existing info)
        draw_hud(screen, v_left, v_right, math.degrees(robot_theta))

        # ADD NOISE DISPLAY (UNDER HUD)
        noise_text = font_med.render(
            f"Noise level: {NOISE_LEVEL:.2f}", True, GREEN
        )
        screen.blit(noise_text, (10, 10 + 3 * 22))
