...
# ARS Assignment - Part 3 - Group 13: Mobile Robot Simulator + EKF Localization + Occupancy Grid Mapping
...
pygame.display.set_caption("ARS Assignment - Part 3 - Group 13: Localization + Occupancy Grid Mapping")
...
#  OCCUPANCY GRID MAPPING PARAMETERS - Assignment Part 3
#  The map is a 2D grid. Each cell stores log-odds of being occupied.
#  log_odds = 0 corresponds to p = 0.5, meaning unknown / no evidence.
MAP_CELL_SIZE = 10                 # pixels per grid cell. 
GRID_COLS = math.ceil(WIDTH / MAP_CELL_SIZE)
GRID_ROWS = math.ceil(HEIGHT / MAP_CELL_SIZE)

P_PRIOR = 0.5                      # no evidence
P_OCC   = 0.75                     # evidence that a cell is occupied
P_FREE  = 0.35                     # evidence that a cell is free

L_PRIOR = math.log(P_PRIOR / (1.0 - P_PRIOR))
L_OCC   = math.log(P_OCC   / (1.0 - P_OCC))
L_FREE  = math.log(P_FREE  / (1.0 - P_FREE))

LOG_ODDS_MIN = -5.0                # clamp to avoid extreme probabilities
LOG_ODDS_MAX =  5.0
...
#  OCCUPANCY GRID MAPPING HELPERS
#  These functions implement the mapping part of the assignment.
#  The inverse sensor model is simple:
#  - cells along the sensor ray before the obstacle are updated as free
#  - the last cell is updated as occupied only if the sensor actually hit something

def create_occupancy_grid():
    """Create an empty log-odds occupancy grid. 0 means p=0.5 / unknown."""
    return np.zeros((GRID_ROWS, GRID_COLS), dtype=np.float32)


def log_odds_to_probability(log_odds_grid):
    """Convert log-odds values back to occupancy probabilities."""
    return 1.0 - 1.0 / (1.0 + np.exp(log_odds_grid))


def world_to_grid(x, y):
    """Convert world/pixel coordinates to grid row and column."""
...
def in_grid(row, col):
    """Check whether a grid cell index is inside the map."""
    return 0 <= row < GRID_ROWS and 0 <= col < GRID_COLS


def cells_along_line(x0, y0, x1, y1):
    """
    Return grid cells touched by a line segment from (x0, y0) to (x1, y1).
    This sampling-based version is simple and sufficient for the assignment.
    """
    dx = x1 - x0
    dy = y1 - y0
    ...
    if length < 1e-9:
        row, col = world_to_grid(x0, y0)
        return [(row, col)] if in_grid(row, col) else []

    # sample often enough so that we do not skip small grid cells
    ...
    for s in range(steps + 1):
        ...
        if in_grid(row, col) and (row, col) != last_cell:
...
def update_occupancy_grid(log_odds_grid, pose, distances):
    """
    Update the occupancy grid using the current pose and the 12 distance sensors.

    pose      : [x, y, theta], usually the EKF estimate for Part 3
    distances : list of measured distances from get_sensor_readings()
    """
    robot_x, robot_y, robot_theta = pose
    max_reading = MAX_SENSOR_DISTANCE - CIRCLE_RADIUS

    # The robot's current cell must be free because the robot occupies that position.
    robot_row, robot_col = world_to_grid(robot_x, robot_y)
    if in_grid(robot_row, robot_col):
        log_odds_grid[robot_row, robot_col] += L_FREE

    for i, dist in enumerate(distances):
        angle = robot_theta + i * SENSOR_ANGLE_STEP

        # Sensor origin is placed on the robot edge, same convention as draw_sensors().
        sx = robot_x + math.cos(angle) * CIRCLE_RADIUS
        sy = robot_y + math.sin(angle) * CIRCLE_RADIUS

        # If the measured distance is close to max range, we treat it as no obstacle hit.
        measured_dist = max(0.0, min(float(dist), max_reading))
        hit_detected = measured_dist < (max_reading - 1.0)

        ex = sx + math.cos(angle) * measured_dist
        ey = sy + math.sin(angle) * measured_dist

        ray_cells = cells_along_line(sx, sy, ex, ey)
        if not ray_cells:
            continue

        # Cells before the end of the ray are free.
        free_cells = ray_cells[:-1] if hit_detected else ray_cells
        for row, col in set(free_cells):
            log_odds_grid[row, col] += L_FREE - L_PRIOR

        # End cell is occupied only when the sensor actually saw an obstacle.
        if hit_detected:
            end_row, end_col = ray_cells[-1]
            if in_grid(end_row, end_col):
                log_odds_grid[end_row, end_col] += L_OCC - L_PRIOR

    # Clamp the grid to keep values numerically stable.
    np.clip(log_odds_grid, LOG_ODDS_MIN, LOG_ODDS_MAX, out=log_odds_grid)


def draw_occupancy_grid(surface, log_odds_grid):
    """
    Draw the occupancy grid as a background:
    - dark cells   : likely occupied
    - light cells  : likely free
    - gray cells   : unknown
    """
    probs = log_odds_to_probability(log_odds_grid)
    ...
    for row in range(GRID_ROWS):
        for col in range(GRID_COLS):
            ...
            if p_occ > 0.55:
                # occupied -> darker
                shade = max(0, min(255, int(255 * (1.0 - p_occ))))
            elif p_occ < 0.45:
                # free -> very light
                shade = 245
            else:
...
def draw_hud(surface, v_left, v_right, theta_deg, num_obs=0, map_mode="EKF", show_map=True):
    ...
    lines = [
        ...
        f"Landmarks   : {num_obs}",
        f"Mapping pose: {map_mode}",
        f"Map visible : {'yes' if show_map else 'no'}",
        "Keys: M map pose | G show map | C clear map | T kidnap",
    ]
...
def main():
    ...
    # Part 3: occupancy grid map
    occupancy_log_odds = create_occupancy_grid()
    use_ekf_pose_for_mapping = True   # True: mapping uses EKF pose, False: mapping uses ground-truth pose
    show_map = True
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_t:
                        # Teleport robot to random position (kidnap)
                        robot_x, robot_y, robot_theta = teleport_valid_location()
                        trajectory_actual.clear()
                        trajectory_estimated.clear()

                if event.key == pygame.K_c:
                        # Clear only the map, useful for repeated experiments.
                        occupancy_log_odds[:] = 0.0

                if event.key == pygame.K_m:
                        # Toggle between mapping with EKF pose and mapping with true pose.
                        # True pose is useful for debugging the mapping algorithm.
                        use_ekf_pose_for_mapping = not use_ekf_pose_for_mapping

                if event.key == pygame.K_g:
                        # Show/hide occupancy grid background.
        ...
        # Use the measured distances, but choose whether the pose comes from EKF or ground truth.
        # For the assignment, EKF pose is the recommended mode because mapping and localization run in parallel.
        if use_ekf_pose_for_mapping:
            mapping_pose = kf_mu
            map_mode = "EKF estimate"
        else:
            mapping_pose = np.array([robot_x, robot_y, robot_theta])
            map_mode = "true pose"

        update_occupancy_grid(occupancy_log_odds, mapping_pose, distances)

        #   Rendering                      
        screen.fill(WHITE)
        if show_map:
            draw_occupancy_grid(screen, occupancy_log_odds)
        draw_walls(screen)
        ...
        draw_hud(screen, v_left, v_right,
                 ...
                 map_mode=map_mode, show_map=show_map)
